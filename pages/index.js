...
const App = () => {
  ...
  // New function to concatenate the greeting
  const handleConcatenateGreeting = async () => {
    try {
      const tx = await contract.concatenateGreeting(additionalText);
      await tx.wait();
      const updatedGreeting = await contract.getGreeting();
      setGreeting(updatedGreeting);
      setAdditionalText("");
    } catch (error) {
      console.error("Concatenate Greeting error:", error);
    }
  };
    ...
    <div style={containerStyle}>
      <div style={boxStyle}>
        ...
        <div>
          ...
          />
          <button onClick={handleConcatenateGreeting}>Concatenate Greeting</button>
        </div>
